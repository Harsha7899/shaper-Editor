    ...
    if (selectedShape && transformMode) {
        ...
        switch (transformMode) {
            ...
            case 'scale':
                ...
                let direction = (dx + dy) > 0 ? 1 : -1;
                let scaleAmount = 1 + (direction * 0.01); // scale ±1% based on drag direction
                selectedShape.scale(scaleAmount, scaleAmount);
                dragStartX = x;
                dragStartY = y;
...
// === Text Shape Support ===
class TextShape {
    constructor(x, y, text = "Sample Text", color = "black", fontSize = 20) {
        this.x = x;
        this.y = y;
        this.text = text;
        this.color = color;
    ...
    contains(px, py) {
        const width = this.text.length * this.fontSize * 0.5;
    ...
    scale(sx, sy) {
        this.fontSize *= (sx + sy) / 2;
    }

    rotate(angle) {
        // Rotation not meaningful for simple text here
    }
...
// === Attribute Editor Support ===
function updateAttribute(attribute, value) {
    if (!selectedShapes.length) return;
    selectedShapes.forEach(shape => {
        if (attribute === 'color' && 'color' in shape) shape.color = value;
        if (attribute === 'thickness' && 'thickness' in shape) shape.thickness = parseInt(value);
        if (attribute === 'fontSize' && shape.type === 'text') shape.fontSize = parseInt(value);
    });
    redraw();
}

// === Export to LaTeX Support ===
function exportToLaTeX() {
    let latex = "\\documentclass{article}\\usepackage{tikz}\\begin{document}\\begin{tikzpicture}\n";

    shapes.forEach(shape => {
        if (shape.type === "line") {
            latex += `\draw[line width=${shape.thickness}pt, ${shape.color}] (${shape.x1/10},${-shape.y1/10}) -- (${shape.x2/10},${-shape.y2/10});\n`;
        } else if (shape.type === "text") {
            latex += `\node[anchor=west, text=${shape.color}] at (${shape.x/10},${-shape.y/10}) {${shape.text}};\n`;
        } else {
            latex += `% Unsupported shape (${shape.type})\n`;
        }
    });
    latex += "\end{tikzpicture}\end{document}";
    const blob = new Blob([latex], { type: "text/plain;charset=utf-8" });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = "shapes_export.tex";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
}
